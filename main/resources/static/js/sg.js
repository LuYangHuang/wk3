...
(function ($) {
    ...
    $.tmDialog = {
        ...
        _init: function (options) {
            ...
            if (opts.showHeader) {
                $dialog.find(".tmui-modal-header").hide();
            }

            if (eval(opts.showIcon)) {
                $dialog.find(".tmui-modal-content").width(opts.width - 148);
            } else {
                $dialog.find(".tmui-modal-content").css({
                    "width": (opts.width - 100),
                    "textAlign": opts.arrow || "center",
                    "padding": 0,
                    "padding": 50
                });
            }
        ...
        window: function (options) {
            ...
            if (!options.ajax) {
                ...
                $window.find(".tmui-modal-body").css({
                    "overflowX": "hidden",
                    "overflowY": "auto",
            ...
            if (!options.showHeader) {
                $window.find(".tmui-modal-header").css({
                    "position": "absolute",
                    "width": "100%",
                    "height": 47,
                    "zIndex": 2
    ...
    $.fn.tmDrag = function (settings) {
        return this.each(function () {
            var $drag = $(this),
                options = $.extend({}, $.fn.tmDrag.defaults, $.fn.tmDrag.defaults.parseOptions(this), settings),
                // 拖动对象
            ...
            $handle.mousedown(function (e) {
                ...
                if (options.ghsot) {
                    ghsotDiv = options.ghsotEvent($drag);
                }
                if (!isDraging) {
                    _ondragstrart(e);
                    return false;
    ...
    $.tpAjax = {
        request: function (options, dataJson) {
            var opts = $.extend({}, {
                limit: true, beforeSend: function () {
        ...
        ajaxMain: function (opts, _url, dataJson) {
            $.ajax({
                ...
                error: function () {
                    tmLoading("抱歉！因为操作不能够及时响应，请稍后在试...", 1);
                    opts.error();
                    clearTimeout(ajaxTimeout);
                },
